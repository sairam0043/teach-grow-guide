...
router.post('/create-course-order', async (req, res) => {
  try {
    ...
    const secureAmount = purchaseType === 'assessment' ? 100 : 1500;
    const amountInPaise = Math.round(secureAmount * 100);
      ...
      const emailHtml = coursePayment.purchaseType === 'full_course'
        ...
        : `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px; background-color: #ffffff;">
            ...
            <p>Hello <strong>${coursePayment.studentName}</strong>,</p>
            <p>Thank you for registering for the <strong>AI Future Skills Program</strong> assessment. Your payment of <strong>₹100</strong> has been successfully processed.</p>
